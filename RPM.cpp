...
unsigned int getTotalNumberOfNeigboringPatches(const Index_Mesh* const mesh, const unsigned int* const& jumpToFacesPerVertexEnd, const unsigned int* const& faceIndicesPerVertex)
{

	unsigned int index, v1, v2, v3, a, b, c;
	unsigned int jumpIndexInFacesPerVertex;
	bool one;
	unsigned int numOfNeighboringFaces = 0;
	for (unsigned int faceIndex = 0; faceIndex < mesh->numOfFaces; ++faceIndex)
	{
		index = faceIndex * 3;
		v1 = mesh->indices[index];
	...
	return numOfNeighboringFaces;
}

const unsigned short MAX_PATCH_SIZE = 256;
const int NUMBER_OF_SETUP_BLOCKS = 1;
const int INITIAL_PATCH_SIZE_WITHOUT_NEIGHBORING_PATCHES = 4 + NUMBER_OF_SETUP_BLOCKS; //1 Counters + 3 Outer-Vertices + 0 Innter-Vertices + 3 Connetced Patches + 1 Face
...
{
	unsigned int currentPosition, index, v1, v2, v3, a, b, c;
	unsigned int jumpIndexInFacesPerVertex;
	unsigned char numOfNeigboringPatches;

	bool one;
	jumpToPatchesStart = new unsigned int[mesh->numOfFaces + 1];
	jumpToPatchesStart[0] = 0;
	patches = new unsigned int[mesh->numOfFaces * INITIAL_PATCH_SIZE_WITHOUT_NEIGHBORING_PATCHES + getTotalNumberOfNeigboringPatches(mesh,jumpToFacesPerVertexEnd,faceIndicesPerVertex)];
	currentPosition= -1;
	for (unsigned int faceIndex = 0; faceIndex < mesh->numOfFaces; ++faceIndex)
	{
		index = faceIndex * 3;
		...
		//The setup block cannot be set at this point, since the number of neighboring patches are not yet known
		currentPosition += NUMBER_OF_SETUP_BLOCKS;
		patches[++currentPosition] = v1;
		patches[++currentPosition] = v2;
		patches[++currentPosition] = v3;

		numOfNeigboringPatches = 0;
		...
		patches[jumpToPatchesStart[faceIndex]] = Patch::setSetupBlock(3, 0, numOfNeigboringPatches);
		jumpToPatchesStart[faceIndex + 1] = currentPosition + 1;
	}
}
...
unsigned int getTotalNumberOfNeigboringPatches(unsigned int*& jumpToPatchesStart, unsigned int*&  patches, unsigned int& numOfPatches)
{
	unsigned int res = 0;
	Patch p;
	for (unsigned int i = 0; i < numOfPatches; ++i)
	{
		p.setup(i, jumpToPatchesStart, patches);
		res += p.numNeighborPatches;
	}
	return res / 2;
}
	...
	std::vector<float> weights)
{
	...
	float* qualities = new float[getTotalNumberOfNeigboringPatches(jumpToPatchesStart, patches, numOfPatches)];
	unsigned int numQualities;
	jumpToPatchesStart2[0] = 0;
	unsigned int* patches2 = new unsigned int[jumpToPatchesStart[numOfPatches]];
					...
					if (!Patch::isMerge(a.neighborPatches[j], jumpToPatchesStart, patches))
					{
						...
						if (!merged->mergePatchIsBiggerMaxPatchSize())
						{
							...
							if (a.patchIndex < b.patchIndex)
								qualities[++numQualities] = currentQuality;
...
Index_Mesh* rpm(Index_Mesh* mesh)
{
	...
	for ( int i = 0; i < exp; ++i)
	{
		...
		{
			for (int h = 0; h < exp - (i + j); ++h)
			{
				for (int k = 0; k < exp - (i + j + h); ++k)
				{
					...
					numSettings = 0;
					if (i != 0)
						settings[numSettings++] = i;
					if (j != 0)
						settings[numSettings++] = j;
					if (h != 0)
						settings[numSettings++] = h;
					if (k != 0)
						settings[numSettings++] = k;
					int z = 0;
					for (; z < numSettings; ++z)
					{
						if (settings[z] == 1)
					...
					if (z == numSettings)
					{
						...
						for (; z < exp; ++z)
						{
							...
							for (; y < numSettings; ++y)
							{
								if (commonDinominator == -1)
									commonDinominator = settings[y] % z;
								else
								{
									if (commonDinominator != settings[y] % z)
										break;
								}
							}
							if (y == numSettings)
					...
					if (!hasBeenTried)
					{
						...
						for (int g = 0; g < numOfPatches; ++g)
						{
							...
							ave += 255 - (a.numInnerVertices + a.numOuterVertices);
							ave2 += 1.0 * a.numOuterVertices / (a.numOuterVertices + a.numInnerVertices);
						}
						ave /= numOfPatches;
						ave2 /= numOfPatches;
						if (ave2 < best)
...
int main()
{
	...
	unsigned int i = -1;
	std::cout << i;
	//Index_Mesh* m = load_ply("C:/Users/Hendrik Annuth/Desktop/dragon.ply");
